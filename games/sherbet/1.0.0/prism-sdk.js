...
(function (global) {
   ...
   *     gameplayStart: () => portal.gameplayStart(),
   *     gameplayStop: () => portal.gameplayStop(),
   *     loadingFinished: () => portal.loadingFinished()
   *   });
   *
   * The three lifecycle hooks are not optional at every portal: CrazyGames
   * requires a gameplayStart event for a Full Launch, and Poki uses the same
   * pair to decide when it may interrupt. They are fire-and-forget here — a
   * game must never await them, and a portal that throws must never be felt.
   *
  ...
  function makeAds() {
    ...
    // Lifecycle signals are announcements, not requests: nothing waits on them
    // and nothing they throw may reach the game.
    var announce = function (kind) {
      if (!provider || typeof provider[kind] !== 'function') return;
      try {
        var result = provider[kind]();
        if (result && typeof result.catch === 'function') result.catch(function () {});
      } catch (err) { /* a portal's bookkeeping is never the game's problem */ }
    };
    ...
    return {
      ...
      gameplayStart: function () { announce('gameplayStart'); },
      gameplayStop: function () { announce('gameplayStop'); },
      loadingFinished: function () { announce('loadingFinished'); }
    };
  }
